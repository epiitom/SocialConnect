...
export async function GET(
  ...
  return withAuth(request, async (req, currentUser) => {
    try {
      ...
      const { data: followers, error, count } = await (await supabase)
        .from('follows')
        .select(`
          follower_id,
          created_at,
      ...
      if (error) {
        return NextResponse.json(
          { error: 'Failed to fetch followers', message: error.message },
          { status: 500 }
        );
      }
      ...
      const followersWithStatus = await Promise.all(
        (followers || []).map(async (follow: any) => {
          const { data: isFollowing } = await (await supabase)
            ...
            .eq('following_id', follow.follower.id)
            .single();
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
      ...
      return NextResponse.json(
        { error: 'Internal server error', message: 'Failed to fetch followers' },
        { status: 500 }
      );
    }
  });
